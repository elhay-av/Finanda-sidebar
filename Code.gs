...
function onOpen() {
  ...
  ui.createMenu("משיכת מידע").addItem("משיכת מידע", "UpdateByRange").addToUi();
}
...
function showExpansesFormDialog() {
  var html = HtmlService.createHtmlOutput(
    '<iframe height="100%" width="100%" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRunJmjpGvKAZnQQdKhP9T9Wn4hvnhd2DkLJDQAbB68aQSlwT3KnP3bg-nqFVCdDXR_pfCNHPK7mSB_/pubhtml?gid=818273642&amp;single=true&amp;widget=true&amp;headers=false"></iframe>',
  )
    .setWidth(400)
    .setHeight(600);
  getProtectedUi().showModalDialog(html, "הוצאות חריגות");
}
...
function updateCellFromSideBar(
  Amount,
  Description,
  groupId,
  columnIndex,
  ...
  const targetSheet = year
    ? getProtectedActiveSpreadsheet().getSheetByName(year.toString())
    : getProtectedActiveSpreadsheet().getActiveSheet();
  if (!targetSheet) {
...
function updateCell(range, transactions) {
  if (transactions && transactions.length) {
    ...
    let notes = transactions.map(
      (i) => `${i.Description}: ${Math.abs(i.Amount)}`,
    );

    Logger.log(
      `[updateCell] Row: ${range.getRowIndex()}, Column: ${range.getColumn()}: =${sum.join("+")} note: ${notes.join("\n")}`,
    );
    ...
    const currentValues = range
      ...
      .split("+")
      .map(Number);
    const mergedValues = [...new Set([...currentValues, ...sum])].filter(
      Boolean,
    );
    ...
    if (currentNote) {
      ...
      const notesString = [currentNote, ...filteredNotes]
        .filter(Boolean)
        .join(" \n");
...
function updateSheetData(income, expanses, accountsMap, year, month) {
  ...
  Logger.log(
    `[updateSheetData] Starting update for sheet: ${year}, column: ${actualColumnIndex}, rows: ${startRow} to ${lastRow}`,
  );
